    ...
    <div className="p-8 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <select
          ...
          className="px-3 py-2 rounded-xl bg-[#0b101b] border border-[#1a273e] text-white text-xs"
        >
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {markets.map((m) => (
            <div
              key={m.id}
              className="bg-[#0b101b] border border-[#1a273e] rounded-2xl p-5 space-y-4 shadow-xl"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {m.outcomes?.map((outcome) => {
                  ...
                  return (
                    <div
                      key={outcome.id}
                      className="bg-[#101726] border border-[#1e2d45] rounded-xl p-3 flex flex-col justify-between gap-2"
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-white">{outcome.name}</span>
                        <span
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                          <input
                            ...
                            className="w-full px-2.5 py-1.5 rounded-lg bg-[#0b101b] border border-[#1a273e] text-[#00e676] font-mono font-bold text-xs"
                          />
                        </div>
                        <button
                          onClick={() => {
                          ...
                          className="px-2.5 py-1.5 rounded-lg bg-[#162238] text-white text-xs hover:bg-[#00e676] hover:text-black font-bold transition-all"
                        >
                          Push
                        </button>
                      </div>
                      ...
                      {m.status !== 'SETTLED' && (
                        <button
                          ...
                          className="text-[11px] text-[#00e676] font-bold hover:underline text-left mt-1"
                        >
                          ✓ Settle as Winner
                        </button>
                      )}
